...
$safe_item_id = mysqli_real_escape_string($mysqli, $_GET['item_id']);
 ...
 $get_item_sql = "SELECT c.id as cat_id, c.cat_title, si.item_title, si.item_price, si.item_desc, si.item_image FROM store_items AS si LEFT JOIN store_categories AS c on c.id = si.cat_id
...
if (mysqli_num_rows($get_colors_res) > 0) {
 $display_block .= "<p><strong><label for=\"sel_item_color\">Available Colors:</strong></label><br/><select id=\"sel_item_color\" name=\"sel_item_color\">";
...
<style>
body {background-color: #C6AEF6;font-family: verdana; }
h1   {color:#423067;}
